...
/* .then(async () => {
  ...
  const dataPeriodos = [
    ...
    },
    {
      ...
      fin: moment('26/11/2019', 'DD/MM/YYYY').format('YYYY-MM-DD HH:mm:ssZ'),
  ...
  // Crear Cargas
  const carga1 = await db.Cargas.create({});
  const carga2 = await db.Cargas.create({});
  const carga3 = await db.Cargas.create({});
  const carga4 = await db.Cargas.create({});
  const carga5 = await db.Cargas.create({});
  const carga6 = await db.Cargas.create({});
  const carga7 = await db.Cargas.create({});
  const carga8 = await db.Cargas.create({});
  const carga9 = await db.Cargas.create({});
  const carga10 = await db.Cargas.create({});
